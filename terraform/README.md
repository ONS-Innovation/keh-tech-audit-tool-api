...
Set these Lambda-specific variables in the `terraform/lambda/env/<environment>/<environment>.tfvars` file before applying:
...
`AWS_ACCOUNT_NAME` in the Lambda runtime is derived directly from `domain`.
